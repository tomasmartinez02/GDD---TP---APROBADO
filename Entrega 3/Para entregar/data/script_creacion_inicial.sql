...
BEGIN
    ...
    SELECT DISTINCT PAGO_ALQUILER_NRO_PERIODO, PAGO_ALQUILER_FEC_INI, PAGO_ALQUILER_FEC_FIN
    FROM gd_esquema.Maestra
    WHERE PAGO_ALQUILER_CODIGO IS NOT NULL;
END
GO
...
BEGIN 
    ...
    SELECT DISTINCT a.id, DETALLE_ALQ_NRO_PERIODO_INI, DETALLE_ALQ_NRO_PERIODO_FIN, DETALLE_ALQ_PRECIO
    FROM gd_esquema.Maestra m
	JOIN GRUPO_1.inquilinos i ON M.INQUILINO_DNI = i.dni AND M.INQUILINO_NOMBRE = i.nombre AND M.INQUILINO_APELLIDO = i.apellido
	JOIN alquileres a ON (m.ALQUILER_CODIGO = a.codigoAlquiler AND a.inquilino_id = i.idInquilino)
...
BEGIN
    ...
    FROM gd_esquema.Maestra M
	JOIN GRUPO_1.inquilinos i ON M.INQUILINO_DNI = i.dni AND M.INQUILINO_NOMBRE = i.nombre AND M.INQUILINO_APELLIDO = i.apellido
    JOIN GRUPO_1.alquileres a ON (M.ALQUILER_CODIGO = a.codigoAlquiler AND a.inquilino_id = i.idInquilino)
